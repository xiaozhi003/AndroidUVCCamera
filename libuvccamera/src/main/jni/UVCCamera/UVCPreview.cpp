...
int UVCPreview::startPreview() {
	...
	if (!isRunning()) {
		...
		pthread_mutex_lock(&preview_mutex);
		{
		    // 即使没有传入预览window也保证预览线程运行，外部出yuv图像通过opengl预览
			//if (LIKELY(mPreviewWindow)) {
...
int UVCPreview::stopPreview() {
	...
	if (LIKELY(b)) {
        ...
        if (mHasCaptureThread) {
            pthread_cond_signal(&capture_sync);
            LOGI("pthread_join capture_thread...");
            if (pthread_join(capture_thread, NULL) != EXIT_SUCCESS) {
                LOGW("UVCPreview::terminate capture thread: pthread_join failed");
            }
        }
		LOGI("pthread_join preview_thread...");
		pthread_cond_signal(&preview_sync);
...
uvc_frame_t *UVCPreview::waitPreviewFrame() {
	...
	pthread_mutex_lock(&preview_mutex);
	{
		if (!previewFrames.size()) {
            struct timespec timeout = pthread_wait_timeout(1000);
			pthread_cond_timedwait(&preview_sync, &preview_mutex, &timeout);
		}
...
uvc_frame_t *UVCPreview::waitCaptureFrame() {
	...
	pthread_mutex_lock(&capture_mutex);
	{
		if (!captureQueu) {
            struct timespec timeout = pthread_wait_timeout(1000);
			pthread_cond_timedwait(&capture_sync, &capture_mutex, &timeout);
		}
