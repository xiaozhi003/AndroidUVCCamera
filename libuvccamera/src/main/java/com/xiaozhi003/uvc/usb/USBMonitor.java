...
import com.xiaozhi003.uvc.util.BuildCheck;
import com.xiaozhi003.uvc.util.HandlerThreadHandler;
import com.xiaozhi003.uvc.util.HexUtils;

public final class USBMonitor {

    public static final String ACTION_USB_DEVICE_ATTACHED = "android.hardware.usb.action.USB_DEVICE_ATTACHED";
    private static boolean DEBUG = false;    // TODO set false on production
        ...
        public void run() {
            ...
            if ((n > mDeviceCounts) || (m > hasPermissionCounts)) {
                ...
                if (mOnDeviceConnectListener != null) {
                    if (mOnDeviceConnectListener instanceof OnDeviceConnectListenerX) {
                        ((OnDeviceConnectListenerX) mOnDeviceConnectListener).onAttachAll(devices);
                    }
                    for (int i = 0; i < n; i++) {
...
//            mAsyncHandler.postDelayed(this, 2000);    // confirm every 2 seconds
    ...
    public static final String getDeviceKeyName(final UsbDevice device, final String serial, final boolean useNewAPI) {
        ...
        if (useNewAPI && BuildCheck.isAndroid5()) {
            sb.append("#");
            // 修复当targetSDKVersion>=29无权限获取SerialNumber信息
//			if (TextUtils.isEmpty(serial)) {
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
    ...
    public static class OnDeviceConnectListenerX implements OnDeviceConnectListener {

        public void onAttachAll(List<UsbDevice> devices) {
...
//            mInfo = updateDeviceInfo(monitor.mUsbManager, device, null);
            ...
            // attach后打开设备，但未关闭连接，导致fd句柄一直占用（可能导致设备一直处于连接状态无法释放，灯长亮等）
            // attach后，不连接设备，保证设备只有一个连接
//            mConnection = monitor.mUsbManager.openDevice(device);
            if (mConnection != null) {
                final int desc = mConnection.getFileDescriptor();
                final byte[] rawDesc = mConnection.getRawDescriptors();
                int fd = mConnection.getFileDescriptor();
                Log.i(TAG, "init open fd: " + fd);
...
//			mInfo = updateDeviceInfo(monitor.mUsbManager, device, null);
            ...
            Log.i(TAG, "UsbDeviceConnection:open:fd=" + mConnection.getFileDescriptor() + ", name=" + device.getDeviceName());

            // java层直接获取TLV，可以无需c层claimInterface；也可等c层claimInterface获取TLV
//            int interface_id = 0;
//            int altsetting = 0;
//            boolean claim = mConnection.claimInterface(getInterface(interface_id, altsetting), true);
//            Log.i(TAG, "claim = " + claim);
//            getTlv();
//            releaseInterface(getInterface(interface_id, altsetting));
        }
        ...
        public byte[] getTransformPara() {
            return mTransformPara;
        }

        public void setTransformPara(byte[] transformPara) {
            mTransformPara = transformPara;
        }
         ...
         * @return
         */
        public synchronized int openImage() {
            if (DEBUG) Log.i(TAG, "openImage...");
            if (mConnection == null) {
                return -1;
            }
            int ret;
            ...
            byte[] setData = {(byte) 0x9E, 0x09, 0x01, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x100, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return ret;
            }
        ...
        public synchronized int getPress() {
            ...
            // set data
            byte[] setData = {(byte) 0x9F, 0x09, 0x01, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x100, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return ret;
            ...
            // get data
            byte[] getData = {(byte) 0x9F, 0x09, 0x01, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_GET, 0x81, 0x100, 0x300, getData, getData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "getData ret = " + ret);
                return 0;
            }
         ...
         * @return
         */
        public synchronized int openLight() {
            if (DEBUG) Log.i(TAG, "openLight...");
            if (mConnection == null) {
                return -1;
            }
            int ret;
            ...
            byte[] setData = {(byte) 0x07, 0x10, 0x07, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x100, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return ret;
         ...
         * @return
         */
        public synchronized int closeLight() {
            if (DEBUG) Log.i(TAG, "closeLight...");
            if (mConnection == null) {
                return -1;
            }
            int ret;
            ...
            byte[] setData = {(byte) 0x07, 0x10, 0x06, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x100, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return ret;
        ...
        public synchronized String getECX101FocalLength() {
            if (DEBUG) Log.i(TAG, "getECX101FocalLength...");
            if (mConnection == null) {
                return null;
            }
            // 读写设备sn号，USB_REQ_CS_INTERFACE_GET
            byte[] dat = new byte[64];
            int size = dat.length / 8;

            int ret;
            int transfer_timeout = 1000;

            byte[] setData = {(byte) 0x00, (byte) 0xF1, (byte) 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x300, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return null;
            }
            ...
            for (int i = 0; i < size; i++) {
                ...
                ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_GET, 0x81, 0x300, 0x300, data, data.length, transfer_timeout);

                if (ret < 0) {
                    if (DEBUG) Log.e(TAG, "ret = " + ret);
                    break;
                }
                System.arraycopy(data, 3, dat, i * 8, 8);
            }

            int totalLen = dat[0];
            if (DEBUG) Log.i(TAG, "totalLen:" + totalLen);
            int focalLen = (totalLen - 2) / 2;
            if (focalLen <= 0) {
                return null;
            }
            byte[] focalLength = new byte[focalLen];
            for (int i = 0; i < focalLen; i++) {
                focalLength[i] = dat[2 + 2 * i];
            }

            return new String(focalLength);
        }
        ...
        public synchronized String getECX101SN() {
            if (DEBUG) Log.i(TAG, "getECX101SN...");
            if (mConnection == null) {
                return null;
            }
            // 读写设备sn号，USB_REQ_CS_INTERFACE_GET
            ...
            byte[] setData = {(byte) 0xc0, (byte) 0xF0, (byte) 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x300, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return null;
        ...
        public synchronized void getTlv() {
            if (DEBUG) Log.i(TAG, "getTlv...");
            if (mConnection == null) {
                return;
            }
            // 读写设备sn号，USB_REQ_CS_INTERFACE_GET
            byte[] tag = new byte[4096];
            int size = tag.length / 8;

            int ret;
            int transfer_timeout = 1000;

            byte[] setData = {0x00, (byte) 0xF0, (byte) 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x300, 0x300, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "setData ret = " + ret);
                return;
            ...
            for (int i = 0; i < size; i++) {
                ...
                System.arraycopy(data, 3, tag, i * 8, 8);
            }

            long start = System.currentTimeMillis();
            byte[] verBytes = parseTLV(tag, (byte) 0x03);
            if (verBytes != null) {
                mFirmwareVersion = new String(verBytes);
                if (DEBUG) Log.i(TAG, "Firmware version: " + mFirmwareVersion);
            }
            byte[] snBytes = parseTLV(tag, (byte) 0x04);
            if (snBytes != null) {
                mSN = new String(snBytes);
                if (DEBUG) Log.i(TAG, "SN: " + mSN);
            }

            byte[] licBytes = parseTLV(tag, (byte) 0x0b);
            if (licBytes != null) {
                mLic = HexUtils.encodeHexString(licBytes);
                if (DEBUG) Log.i(TAG, "lic: " + mLic);
            }

            byte[] transformPara = parseTLV(tag, (byte) 0);
            mTransformPara = transformPara;
        ...
        public synchronized int writeToASIC(short addr, byte value) {
            if (DEBUG) Log.i(TAG, "writeToASIC...");
            if (mConnection == null) {
                return -1;
            }
            int ret;
            ...
            byte[] setData = {(byte) addr, (byte) (addr >> 8), value, 0x00};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x01 << 8, 0x3 << 8, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "writeToASIC ret = " + ret);
                return ret;
            }
        ...
        public synchronized byte readToASIC(short addr) {
            ...
            // Dummy Write
            byte[] setData = {(byte) addr, (byte) (addr >> 8), 0x00, (byte) 0xff};
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_SET, 0x01, 0x01 << 8, 0x3 << 8, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "readToASIC write ret = " + ret);
                return 0;
            }

            // Read
            ret = mConnection.controlTransfer(USB_REQ_CS_INTERFACE_GET, 0x81, 0x01 << 8, 0x3 << 8, setData, setData.length, transfer_timeout);
            if (ret < 0) {
                if (DEBUG) Log.e(TAG, "readToASIC ret = " + ret);
                return 0;
            }
        ...
        private byte[] parseTLV(byte[] tlv, byte dev_tag) {
            byte DEV_TAG = dev_tag;
            int tagPos = 2;
            while (tagPos <= tlv.length - 1) {
                byte tag = (byte) (tlv[tagPos] & 0xFF);
                int pos1 = (tagPos + 1);
                int pos2 = (tagPos + 2);
                if (pos1 >= tlv.length || pos2 >= tlv.length) {
                    break;
                }

                int nLen = tlv[pos1] << 8 & 0xFF00 | tlv[pos2] & 0xFF;

//                Log.i(TAG, "tagPos:" + tagPos);
//                Log.i(TAG, "tag:" + tag);
//                Log.i(TAG, "len:" + nLen);
                if (tag == DEV_TAG) {
                    if (tagPos + 3 + nLen > tlv.length - 1) {
                        break;
                    }
                    byte[] data = new byte[nLen];
                    System.arraycopy(tlv, tagPos + 3, data, 0, nLen);
                    return data;
                }
                tagPos = tagPos + 3 + nLen;
            }
            return null;
        }
        ...
        public synchronized void close() {
            ...
            if (mConnection != null) {
                ...
                Log.e(TAG, "UsbDeviceConnection:close:fd=" + mConnection.getFileDescriptor());
                mInterfaces.clear();
                mConnection.close();
                mConnection = null;
